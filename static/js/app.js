/* =========  App Logic - content in XLSX sheets populates drop down menus and name buttons =========
 ...
 *
 * All local asset URLs (static/, pdfjs/, etc.) are built relative to this base.
 */
const AIM_BASE_PATH = (() => {
  try {
    // explicit <base href="..."> (if present)
...
// Always-on default basemap (prevent gray/blank map if there is a timeout)
...
// PDF opening view: default zoom + fit-to-width (more user friendly -> avoids horizontal scrolling)
function withPdfView(url, zoomPct = 100) {
    const [base, frag] = String(url).split('#');

    // Simulate Acrobat "readable" default:
    // - FitH = fit horizontally (fit-to-width -> no corrective horizontal adjustment for user)
 ...
 * - This is needed to ensure that the code returns the names of reports without encoding characters
...
/* ================= HAP Mosaics: provenance + metadata ================= */
...
// Provenance PDFs: AIMFredericton_HAPMosaic_Provenance_19XX.pdf - one per mosaic
...
async function openMosaicMetadataWindow(year) {
    ...
    try {
        ...
        const sheetName = String(year); // xlsx mosaic metadata tab name is the year of mosaic
...
function addLayerForEntry(entry) {
    ...
    // HAP Mosaics Tiled/MapServer tile layer (ArcGIS Tile/MapServer)
    ...
    // Basemap entry (ESRI): basemap token (Imagery/ImageryLabels) 
    if (svc === 'basemap') {
    ...
    // (basemap) hybrid: imagery + labels (explicit type)
    if (svc === 'hybrid') {
        try {
    ...
    // (basemap) vector-street (using mature basemap 'Streets' for now via basemapLayer to avoid API key)
...
fetch('static/data/imagery.json').then(r => r.json()).then(rows => {
    ...
    // 1) Street layer at top checkbox - default ON
    ...
    //  ESRI Transportation reference overlay (instead of basemap) - roads, highways, airports, rail lines
    ...
    // 2) Open Street Map (OSM) toggle near top
    ...
    Object.keys(grouped).sort().forEach(group => {
        ...
        grouped[group].forEach(entry => {
            ...
            // show opacity slider for raster / tiled layers 
            const svc = String(entry.service_type || '').toLowerCase();
            ...
            if (year) {
                ...
                // position of mosaic extent, metadata and provenance buttons
...
// Chrome/Edge sometimes ignores #view/#zoom on first iframe load
// Force a single re-apply of the fragment once iframe finishes loading
if (pdfViewer) {
...
// load archive document list from JSON
...
function populatePdfSelect(decade) {
    ...
    list.forEach(doc => {
        ...
        // Drop the leading term "searchable" from ui_label_1
        ui1 = ui1.replace(/^searchable\s+/i, '').trim();

        // Document dropdown label: ui_label_1 + ui_label_2 
...
function openPdfInTab(filename, url, aimTitle) {
    // allow duplicates and append with duplicate number -> file.pdf, file.pdf (2)
...
/* ---------------- Resizer between PDF document space and map space ---------------- */
(function setupResizer() {
    ...
    resizer.addEventListener('mousedown', (e) => {
        ...
        const onMouseUp = () => {
            ...
            // re-enable map interactions after re-size
            try {
                map.dragging.enable();
...
/* --------------- keeps anything open as-is when site is being used --------*/
